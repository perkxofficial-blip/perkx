...
export default function Toast({ message, type = 'info', onClose, duration = 3000 }: ToastProps) {
  ...
  const bgBorderColors = {
    success: 'bg-[#0d2818] border-[#22c55e]',
    error: 'bg-[#2a0f0f] border-[#ef4444]',
    warning: 'bg-[#2a2008] border-[#eab308]',
    info: 'bg-[#0c1a2e] border-[#3b82f6]',
  };
  ...
  return (
    <div className="fixed top-6 right-6 z-[9999]" style={{ animation: 'slideUp 0.3s ease-out' }}>
      <div className={`flex items-center gap-3 px-5 py-3 rounded-xl border shadow-[0_8px_32px_rgba(0,0,0,0.4)] min-w-[360px] max-w-md ${bgBorderColors[type]}`}>
        ...
        <p className={`text-sm font-medium mb-0 flex-1 ${textColors[type]}`}>
          {message}
        </p>
        <button
          onClick={onClose}
